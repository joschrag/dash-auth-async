...
_QUART_OAUTH_MODULE = "dash_auth_async.quart_client.QuartOAuth2App"
...
async def invalid_authorize_redirect(_, redirect_uri, *args, **kwargs):
...
async def valid_authorize_access_token(*args, **kwargs):
    return {
...
@patch(
    f"{_QUART_OAUTH_MODULE}.authorize_redirect",
    valid_authorize_redirect,
)
@patch(
    f"{_QUART_OAUTH_MODULE}.authorize_access_token",
    valid_authorize_access_token,
)
def test_oaq001_oidc_auth_login_flow_success(dash_br, dash_thread_server, kwargs):
...
def test_oaq002_oidc_auth_login_fail(dash_thread_server, kwargs):
    app = Dash(__name__, backend="quart", **kwargs)
    app.layout = html.Div(
...
def test_oaq003_oidc_auth_login_several_idp(dash_br, dash_thread_server, kwargs):
    app = Dash(__name__, backend="quart", **kwargs)
    app.layout = html.Div(
        [
            dcc.Input(id="input", value="initial value"),
